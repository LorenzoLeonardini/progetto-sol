...
#define PTHREAD_CREATE(thread_t, attr, func, attr_2) \
	PTHREAD_ERROR(pthread_create(thread_t, attr, func, attr_2), "Creating " #thread_t " thread")

#define PTHREAD_MUTEX_INIT_ERR(mutex_t, attr) \
	PTHREAD_ERROR(pthread_mutex_init(mutex_t, attr), "Creating " #mutex_t " mutex")

#define PTHREAD_MUTEX_DESTROY_ERR(mutex_t) \
	PTHREAD_ERROR(pthread_mutex_destroy(mutex_t), "Destroying " #mutex_t " mutex")
...
#define PTHREAD_COND_SIGNAL(cond_t) \
	PTHREAD_ERROR(pthread_cond_signal(cond_t), "Signaling " #cond_t " condition")
